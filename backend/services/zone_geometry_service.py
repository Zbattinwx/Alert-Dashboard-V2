...
class ZoneGeometryService:
    ...
    def _add_to_cache(self, zone_id: str, geometry: Optional[list[PolygonCoords]]):
        """Add geometry to cache. Only caches successful fetches (with geometry)."""
        # Only cache successful fetches - don't cache failures so we retry on next request
    ...
    async def populate_alert_geometry(self, alert: Alert, force: bool = False) -> bool:
        """
        Populate an alert with zone geometry from its UGC codes.

        For zone-based alerts (like watches), we always want to fetch the full
        zone geometry from the API rather than rely on any embedded polygon,
        which may be incomplete.

        Args:
            alert: The Alert object to populate
            force: If True, always populate even if polygon exists

        Returns:
            True if geometry was added, False otherwise
        """
        logger.info(f"[ZONE_GEOM] populate_alert_geometry called for {alert.product_id}")
        logger.info(f"[ZONE_GEOM]   significance={alert.significance}, force={force}")
        logger.info(f"[ZONE_GEOM]   affected_areas ({len(alert.affected_areas or [])}): {alert.affected_areas}")
        logger.info(f"[ZONE_GEOM]   existing polygon: {len(alert.polygon) if alert.polygon else 0} polygons")

        # Skip if no affected areas
        if not alert.affected_areas:
            logger.info("[ZONE_GEOM]   SKIP: no affected_areas")
            return False

        # For zone-based alerts (watches, with significance "A"), always fetch full geometry
        # because embedded polygons from NWWS text may be incomplete
        is_zone_based = alert.significance in ("A", "Y")  # Watch or Advisory
        has_zone_codes = any(self.get_zone_type(ugc) for ugc in alert.affected_areas)

        logger.info(f"[ZONE_GEOM]   is_zone_based={is_zone_based}, has_zone_codes={has_zone_codes}")

        # Skip if alert already has a polygon, unless it's zone-based or forced
        if alert.polygon and not force and not (is_zone_based and has_zone_codes):
            logger.info("[ZONE_GEOM]   SKIP: has polygon and not zone-based")
            return False
        ...
        if not zone_codes:
            logger.info("[ZONE_GEOM]   SKIP: no valid zone codes found")
            return False

        logger.info(f"[ZONE_GEOM]   Fetching geometry for {len(zone_codes)} zones: {zone_codes}")
        ...
        for zone_id, geometry in geometries.items():
            if geometry:
                logger.info(f"[ZONE_GEOM]     {zone_id}: {len(geometry)} polygon(s)")
                all_polygons.extend(geometry)
            else:
                logger.warning(f"[ZONE_GEOM]     {zone_id}: NO GEOMETRY")

        if all_polygons:
            alert.polygon = all_polygons
            logger.info(f"[ZONE_GEOM]   SUCCESS: Added {len(all_polygons)} total polygon(s) to {alert.product_id}")
            return True

        logger.warning(f"[ZONE_GEOM]   FAIL: No polygons collected for {alert.product_id}")
        return False
    ...
    async def populate_multiple_alerts(
        ...
        For zone-based alerts (watches, advisories), we always fetch the full
        zone geometry even if a polygon already exists, since embedded polygons
        from NWWS text may be incomplete.
        ...
        for alert in alerts:
            ...
            # For zone-based alerts (watches/advisories), always fetch full geometry
            is_zone_based = alert.significance in ("A", "Y")  # Watch or Advisory

            # Skip if already has polygon, unless zone-based
