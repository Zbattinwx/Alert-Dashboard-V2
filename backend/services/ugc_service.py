...
def get_display_locations(ugc_codes: list[str], max_display: int | None = None) -> str:
    """
    ...
    Args:
        ...
        max_display: Maximum number of locations to show before truncating.
                     None means show all locations (no truncation).

    Returns:
        Formatted string like "Franklin County, OH; Delaware County, OH; Knox County, OH"
    """
    if not ugc_codes:
        return ""
    ...
    if max_display is None or len(names) <= max_display:
        return "; ".join(names)
    else:
