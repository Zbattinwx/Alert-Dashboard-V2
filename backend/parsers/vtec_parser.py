...
class VTECParser:
    ...
    def build_product_id(cls, vtec_info: VTECInfo) -> str:
        ...
        Format: {phenomenon}.{office}.{etn}
        For watches (significance 'A'), includes 'A' suffix: {phenomenon}A.{office}.{etn}
        ...
        # For watches, append 'A' to phenomenon
        if vtec_info.significance == AlertSignificance.WATCH:
            phenomenon = f"{phenomenon}A"

        # Remove 'K' prefix from office if present
