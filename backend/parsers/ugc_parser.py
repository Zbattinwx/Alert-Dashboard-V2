...
class UGCParser:
    ...
    def parse(cls, text: str) -> UGCData:
        ...
        # Filter out invalid UGC codes (zone/county 000 doesn't exist)
        # Valid codes have numbers 001-999, not 000
        invalid_codes = [c for c in result.ugc_codes if c.endswith("000")]
        if invalid_codes:
            logger.warning(f"Filtering invalid UGC codes (000 is not valid): {invalid_codes}")
            result.ugc_codes = [c for c in result.ugc_codes if not c.endswith("000")]
            result.validation_warnings.append(f"Removed invalid codes: {invalid_codes}")
